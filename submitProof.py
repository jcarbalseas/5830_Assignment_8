...
def merkle_assignment():
    ...
    # Select a random leaf and create a proof for that leaf
    random_leaf_index = 5320 #random.randint(0, num_of_primes - 1) # Generate a random index from primes to claim
    proof = prove_merkle(tree, random_leaf_index)
    print('making proof for index:', random_leaf_index)
    print('prime is:', primes[random_leaf_index])
    print('proof:', proof)
    # input("enter to proceed")
    ...
    if sign_challenge_verify(challenge, addr, sig):
        print('signed challenge verified passed')
        tx_hash = '0x'
...
def convert_leaves(primes_list):
    ...
    # leaves = [prime.to_bytes(byteorder='big', length = 32)for prime in primes_list]
    return leaves
...
def send_signed_msg(proof, random_leaf):
    ...
    # proof = w3.to_bytes(proof)
    # random_leaf = w3.solidity_keccak(['uint256'], [random_leaf])
    # Build the transaction
    txn = w3.eth.contract(address=address, abi=abi).functions.submit(proof, random_leaf).build_transaction({
